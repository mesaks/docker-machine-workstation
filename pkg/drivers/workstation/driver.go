...
func (d *Driver) SetConfigFromFlags(flags drivers.DriverOptions) error {
	d.Memory = flags.Int("workstation-memory-size")
	d.CPU = flags.Int("workstation-cpu-count")
	d.DiskSize = flags.Int("workstation-disk-size")
	d.Boot2DockerURL = flags.String("workstation-boot2docker-url")
	d.ConfigDriveURL = flags.String("workstation-configdrive-url")
	d.ISO = d.ResolveStorePath(isoFilename)
	d.ConfigDriveISO = d.ResolveStorePath(isoConfigDrive)
	d.SetSwarmConfigFromFlags(flags)
	d.SSHUser = flags.String("workstation-ssh-user")
	d.SSHPassword = flags.String("workstation-ssh-password")
	d.SSHPort = 22
	...
	d.NoShare = flags.Bool("workstation-share")
	switch runtime.GOOS {
	...
	case "linux": // TODO Test linux working
		...
		if flags.String("workstation-share-folder") != "" {
			d.ShareFolder = flags.String("workstation-share-folder")
			d.ShareName, d.GuestFolder, d.GuestCompatLink = parseShareFolderLinux(d.ShareFolder)
		}
	...
	case "windows":
		...
		if flags.String("workstation-share-folder") != "" {
			d.ShareFolder = flags.String("workstation-share-folder")
			d.ShareName, d.GuestFolder, d.GuestCompatLink = parseShareFolderWindows(d.ShareFolder)
		}
	}
...
func parseShareFolderLinux(shareFolder string) (string, string, string) {
	split := strings.Split(shareFolder, "\\")
	ShareName := split[len(split)-1]
	GuestFolder := strings.Join([]string{"", ShareName}, "/")
	driveLetter := strings.Replace(strings.ToLower(split[0]), ":", "", 1)
	GuestCompatLink := strings.Join([]string{"", driveLetter, strings.Join(split[1:len(split)], "/")}, "/")

	return ShareName, GuestFolder, GuestCompatLink
}

func parseShareFolderWindows(shareFolder string) (string, string, string) {
	shareFolder = strings.TrimRight(shareFolder, "\\")
	split := strings.Split(shareFolder, "\\")
	ShareName := split[len(split)-1]
	GuestFolder := strings.Join([]string{"", ShareName}, "/")
	driveLetter := strings.Replace(strings.ToLower(split[0]), ":", "", 1)
	GuestCompatLink := strings.Join([]string{"", driveLetter, strings.Join(split[1:len(split)-1], "/")}, "/")
...
func mountSharedFolder(d *Driver) error {
	...
	log.Infof(fmt.Sprintf("ShareName:%s ShareFolder: %s GuestFolder: %s, GuestCompatLink: %s", d.ShareName, d.ShareFolder, d.GuestFolder, d.GuestCompatLink))
	if d.ShareFolder != "" {
